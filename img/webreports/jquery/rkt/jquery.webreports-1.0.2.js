 ...
 * @version		1.0.2 
 *					- #3746 JG 2013.05.22 - Added secure token optional parameter to $.WR.invoke(). 
 *
	...
	LL: {
		...
		getCGI: function () {
			if (this.cgi == undefined) {
				...
				} else if (location.pathname.lastIndexOf('/cs.exe') >= 0) {
					this.cgi = 'cs.exe';
...
jQuery.extend({
	WR: {
		 ...
		 * @param {Boolean} [secure]  Indicates whether a secure token is required for the action/tag/subtag.  If true, a secureRequestToken will automatically be added to the request.  // #3746 JG 2013.04.03:
		 */		 
		llrequest: 'webreports.runservice',
		
		// #3746 JG 2013.04.03:
		//invoke: function(tagdata, subtags, callback, type) {
		invoke: function(tagdata, subtags, callback, type, secure) {
			// shift over if subtags ommitted
			if ($.isFunction(subtags)) {
				secure = type; // #3746 JG 2013.04.03:
				type = callback;
				callback = subtags;
				subtags = '';
			};
			...
			if (secure) {
				...
				var sparms = {
					func: this.llrequest,
					servicetype: 'getsecuretoken',
					responseType: 'json'
				};
			
				// First request to get Token:
				$.LL.queueAjax(function () {
					$.post($.LL.cgiPath(), sparms, function(data){
						if (data.error || data == "?") {
							alert('An error ocurred while retrieving a Secure Request Token');
						} else {
							// Now fire the original request and include the Token:
							var parms = {
								...
								secureRequestToken: decodeURIComponent(data.content)
							};
...
$.extend(Date.prototype, {
	fromLLDate: function(llDateString) {
		this.setTime( Date.parse(llDateString.substr(2).replace(':', ' ')) );
	},
...
/* Example - use a subtag to retrieve node info:
$.WR.invoke(2000, 'NODEINFO:NAME', function(data) {
	alert(data); // Enterprise
});
*/

/* Example - use a subtag to retrieve node info when a secure request token is required:
$.WR.invoke(2000, 'NODEINFO:NAME', function(data) {
	alert(data); // Enterprise
},true);
*/
...
$.WR.invoke('USERID', 'USERINFO:NAME', function(data) {
	alert(data.content); // {"error":false,"content":"Admin"}
}, 'json');
*/

/* Example - get the current user when USERINFO requires a secure request token:
$.WR.invoke('USERID', 'USERINFO:NAME', function(data) {
	alert(data.content); // {"error":false,"content":"Admin"}
}, 'json', true);
*/
