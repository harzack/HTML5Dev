...
CHECKTREE v1.0 RC (c) 2004 Angus Turnbull, http://www.twinhelix.com
...
CheckTree.prototype.click = function(e) { with (this)
{
 ...
 if (!e[evtProcessed] && elm.id && elm.id.match(/^check-(.*)/))
 {
  ...
  if (tree) tree.setBoxStates(e, true, true);
 }
...
window.onload = function()
{
 ...
 // the for loop cause some javascript warning so calling first one directly
 CheckTree.list['checkmenu'].init();
};
