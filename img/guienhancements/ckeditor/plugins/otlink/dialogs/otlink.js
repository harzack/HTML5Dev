CKEDITOR.dialog.add( 'otlink', function( editor )
{
	...
	{
		var dialog = this.getDialog(),
			partIds = [ 'CSObjectOptions', 'urlOptions', 'anchorOptions', 'emailOptions' ],
			typeValue = this.getValue(),
			uploadTab = dialog.definition.getContents( 'upload' ),
			uploadInitiallyHidden = uploadTab && uploadTab.hidden;

		// hide all tabs for content server object
		if ( typeValue == 'CSObject' )
		{
	...
	var commonLang = editor.lang.common,
		...
		otLinkLang = editor.lang.langOTLink;

	
	var llBase = editor.config['llBase'];
	var nodeId = editor.config['CSID'];
	var autocomplete = '<div>';
	autocomplete += '<input id="pageID" name="pageID" type="hidden" value="0"/>';
	autocomplete += '<div id="autous-popup" class="autocomplete" style="margin-left:0px;margin-top:-10px;"></div>';
	autocomplete += '</div>';
	
	function OpenTargetBrowse()
	{
		OpenBrowserWindow( llBase + '?func=ll&objType=150&objAction=targetBrowse&headerLabel=' + encodeURIComponent( otLinkLang.OTSelectCSObject )+ '&selectLabel=' + encodeURIComponent( otLinkLang.OTSelect )+'&formname=llLinkForm&fieldprefix=LLObject', 600, 400 );
	}
				...
				[
					{
						...
						items :
						[
							[ otLinkLang.OTCSObject, 'CSObject' ],
							[ linkLang.toUrl, 'url' ],
