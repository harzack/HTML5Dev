...
function liveUpdaterUriFunc(id, uri, postFunc, preFunc)
{
    function constructUri()
    {
        ...
        if(uri.indexOf("?") >= 0)
            separator = "&";
        return uri + separator + "name=" + escape(document.getElementById(id).innerHTML);
    }
...
function autocomplete(id, popupId, submitid, uri)
{
    var inputField = document.getElementById(id);
    var popup = document.getElementById(popupId);
    var submitField = document.getElementById(submitid);
    var options = new Array(); 
    var current = 0;
    var originalPopupTop = popup.offsetTop;
    var originalPopupHeight = popup.getAttribute("height", false);
    
    if( originalPopupHeight == null || originalPopupHeight == 0 )
    {
    	originalPopupHeight = 20;    	
    }
    ...
    function constructUri()
    {
        ...
        if(uri.indexOf("?") >= 0)
            separator = "&";
        return uri + separator + "name=" + escape(inputField.value);
    }
     ...
     for (var i = 0; i < oAttribs.length; i++)
    {
        ...
        if (oAttrib.nodeName=='id'){
        	//alert(oAttrib.nodeName + '=' +  oAttrib.nodeValue );
        	submitField.value= oAttrib.nodeValue;
        }        
      }    	
    }
    ...
    function handleClick(e)
    {
      ...
      popup.style.visibility = 'hidden';
      popup.style.height = String(originalPopupHeight) + "px";
      setSubmitField(eventElement(e));
    ...
    function handleOver(e)
    {
      ...
      options[current].className = 'selectedautocomplete';
    }
    ...
    function post()
    {
               ...
               && options[0].innerHTML != inputField.value))
        {
          ...
          options[0].className = 'selectedautocomplete';
        }
        else
        {
          popup.style.visibility = 'hidden';
    ...
    function start(e) {
      ...
      else if((e.keyCode == 13 || e.keyCode == 9) && popup.style.visibility == 'visible')
      {
        ...
        setSubmitField(options[current]);
        popup.style.visibility = 'hidden';
        inputField.focus();
        if(isIE)
        {
