...
var bc_html_menuItemEndDiv =
	'	</DIV>' +
	'	<A TITLE="#ITEMNAME#" HREF="#" onClick="showMenu( \'#MENUNAME#Menu\', \'\' );return false" ><DIV CLASS="#IMGMAPCLASS#"></DIV></A>&nbsp;' +
	bc_html_menuItemClosing;
...
//function bc_itemImg( 'Document', 'webdoc/doc.gif', '2', '144', 0  );
...
function ShowBCDropDownMenu( rootedAt )
{
       ...
       if ( menu != null )
       {
              ...
              menu.style.left = getX( parentNode ) - getX( offsetParent );
              ...
              if ( menu && menu.offsetWidth && menu.offsetHeight )
              {
                     ...
                     if ( document.body.scrollTop + availHeight < ( getY( parentNode ) + menuHeight + parentNode.offsetHeight ) && ( getY( parentNode ) - menuHeight > 0 ) )
                     {
                           menu.style.top = getY( parentNode ) - getY( offsetParent ) - menuHeight;
                     }
                     else
                     {
                           menu.style.top = getY( parentNode ) - getY( offsetParent ) + parentNode.offsetHeight;
                     }
              }
       }
...
function bc_spaceCell( imgStr, wid, directWrite )
{
	...
	var class_browseCheckboxColumn;

	var allChecked = false;
	var changeSort = true;
	var virtual_containerId = "";
	var	classId = "";
	var containerSubType = "";
	var currentPageNum = 1;
	var currentViewType = "3";
	var cookiePath = "";
	var class_columnDefinitionArray;
	var defaultPage = 1;
	var divSeg = 0;
	var doSort = true;
	var editingRow = -1;
	...
	var class_isPaged = true;
	var loggingmessage = "";
	var mUrl = "";
	var nextUrl = "";
	var class_noMultiSelectButtons = true;
	var objFilter = "";
	...
	var class_sortColumn = "name"; // This the default only
	var currentTab = ""
	var totalPages = -1;
	var totalCount = 0;
		...
		if ( null !== currentPageObj && undefined !== currentPageObj.page )
		{
			...
			else
			{
				if ( currentViewType !== currentPageObj.view  )
				{
					...
					class_updateViewType( currentPageObj.view );
	...
	function parsePageHistoryKey( key )
	{
		...
		if ( 4 === arr1.length )
		{
			...
			if ( true === changePage )
			{
				class_getFolderContents( currentPageNum, String.formatLoc( "Jumping to page n", currentPageNum ) );
			}
		}
	}
	
	function class_browseCoreInitialize()
	{
	...
	function class_setBaseUrl ( baseUrlStr )
	{
		class_baseUrl  = baseUrlStr;
	}
	
	function class_setMultiSelectButtons( noMSB )
	{
		class_noMultiSelectButtons = noMSB;

		// go and flip the
		if ( class_browseCheckboxColumn != undefined )
		{
			class_browseCheckboxColumn.SetIsEnabled( !class_noMultiSelectButtons );
		}
	}
	...
	{
		virtual_containerId = dataId;
	}
	
	function setClassId( dataId )
	{
		classId = dataId;
	}

	
	function class_setContainerSubType( subType )
	{
	...
	function class_setSortColumn( sortCol )
	{

		class_sortColumn = sortCol;
	}
	...
	//
	function class_setPageDropDownValues( pageDropdownList )
	{
		...
		{
			exceptionAlert( e, "Error: classbrowse.js/setPageDropDownValues.  Corrupt data in generating list pull down. " );
		}
	}

	function class_setBrowseColumnDefinition( browseColumnDefinitionList )
	{
		...
		try
		{
			...
			if ( localBrowseColumnDefinitionList.ok === true )
			{
				...
				if ( undefined === class_columnDefinitionArray )
				{
					columns = localBrowseColumnDefinitionList.columns;
					
					class_columnDefinitionArray = [];
					...
					for ( i = 0; i < columns.length; i++ )
					{
						if ( columns[i].isDefault )
						{
							...
							{
								eval( "class_columnDefinitionArray[i] = new " + columns[i].columnID + "();" );
								if ( columns[i].columnID == "checkboxColumn" )
								{
									class_browseCheckboxColumn = class_columnDefinitionArray[i];
								}
							}
						}
						else if ( columns[ i ].DisplayAsLink )
						{
							if ( columns[ i ].type == 14 )
							{
								...
								class_columnDefinitionArray[i] = new userColumnWithURL(columns[i].displayName, columns[i].columnName, columns[i].sortable, columns[i].columnEMWidth + "em", columns[i].alignment, columns[i].longText, columns[i].columnName);
							}
							else
							{
								//Non-user type column with link on value
								class_columnDefinitionArray[i] = new columnWithURL(columns[i].displayName, columns[i].columnName, columns[i].sortable, columns[i].columnEMWidth + "em", columns[i].alignment, columns[i].longText, columns[i].columnName, columns[i].URL, columns[i].NewWindow, columns[i].TitleText, columns[i].DisplayValue);
							}
						}
						else
						{
							class_columnDefinitionArray[i] = new arbitraryColumn( columns[i].displayName, columns[i].columnName, columns[i].sortable, columns[i].columnEMWidth + "em", columns[i].alignment, columns[i].longText, columns[i].columnName );	
						}
					}
				}
		...
		{
			exceptionAlert( e, "Error: classbrowse.js/setBrowseColumnDefinition.  Corrupt or missing data in generating column definitions." );
		}
	}
	...
	function ChangeBrowsePageObjectFilter( val, val2  )
	{
		...
		class_getFolderContents( currentPageNum, LocString( "Searching..." ), class_folderContentsSort );
	}
	...
	function ChangeBrowsePageSize( val, val2  )
	{
		...
		if ( val === pageSize && parseInt( val, 10 ) === class_myRows.length )
		{
			return;
		}
		...
		class_getFolderContents( defaultPage, String.formatLoc( "Retrieving page n", defaultPage ), class_folderContentsSort );
	...
	BrowseViewColumn.prototype.SetHeaderParams = function( name, className, scope )
	{
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/BrowseViewColumn.SetHeaderParams." );
		}
	};
		...
		try
		{
			if ( true === this.isEnabled )
			{
				...
				if ( true === this.sort )
				{
					...
					linkText = 'class_sortTableCol( true, "' + this.identifier + '" );return false';
    ...
    BrowseViewColumn.prototype.AddCell = function( rowStruct, dataRow, cellCount, rowNo )
	{
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/BrowseViewColumn.AddCell." );
		}
		return cellCount;
	};
	...
	BrowseViewColumn.prototype.SortComparitor = function (a, b)
	{
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/BrowseViewColumn.SortComparitor." );
		}
	    return 0;
	};
	...
	// getFolderContents()
	BrowseViewColumn.prototype.class_sortTableCol = function ( headerClick )
	{
				...
				if ( true === this.sort )
				{
					if ( changeSort )
					{
						...
						if ( true === this.sortDirectionDesc ) 
						{
							...
							class_sortColumn = ( class_sortColumn !== negColName ) ? negColName : this.colName ;
							
						}
						else
						{
							class_sortColumn = ( class_sortColumn !== this.colName ) ? this.colName : negColName;
						}
						...
						nextUrl = String.format( "%1?func=ll&objid=%2&objAction=browse&sort=%3", class_baseUrl , containerId, class_sortColumn )
					...
					if ( headerClick && totalPages > 1 )
					{
						...
						class_getFolderContents( 1, msgStr );
					}
					else
					{
						class_myRows.sort( this.SortComparitor );

						// if sorting descending then reverse the array
						if ( class_sortColumn === negColName )
						{
							class_myRows.reverse();
						}
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/BrowseViewColumn.class_sortTableCol." );
		}
	};
	...
	function nameWthPrmtdCmdsColumn()
	{
		...
		this.AddCell = function( rowStruct, dataRow, cellCount, rowNo )
		{
			...
			{
				exceptionAlert( e, "Issue occured in classbrowse.js/nameWthPrmtdCmdsColumn.AddCell." );
			}
			return cellCount;
		};
	}
	...
	function columnWithURL(title, colID, sortable, width, alignment, wrapping, dataRowElementName, url, newWindow, titleText, displayValue) 
	{
		...
		this.getCellValue = function(dataRow, rowNo) 
		{
			...
			if (disp == undefined) {
				...
				html = "<a ";
				html += "href=\"" + this.formatValue( url, dataRow, disp, val, true ) + "\" ";
	...
	// Once finished deserializing, it redraws the browseview by calling restoreSortOrder.
	function class_DataStringToVariables( inStringData )
	{	
		var		j;
		var		numRows;
		...
		{
			killPopup();
			exceptionAlert( e, "Error: classbrowse.js/DataStringToVariables.  Corrupt data from the server. " );
			localCopy = null;
		}
		...
		if ( null !== localCopy )
		{
			try
			{
				...
				else
				{
					...
					class_myRows = localCopy.myRows;

					numRows = class_myRows.length;
					...
					for (i=0; i< numRows; i++)
					{
						...
						{
							class_columnDefinitionArray[j].DataConversion( obj );
						}
					}
			...
			{
				killPopup();
				exceptionAlert( e2, "Issue occured in classbrowse.js/DataStringToVariables.  No update of data done. " );
			}
		}
	}
	...
	function getOptionalParams( thisRow )
	{
		...
		if ( undefined !== thisRow && true === thisRow.container )
		{
			if ( class_sortColumn !== "name" )
			{
				...
				{
					optionalParams += "&sort=" + class_sortColumn;
				}
			}
	...
	function makeRowNameCell( thisRow, rowNo )
	{
		...
		{
			outputStr += "<A href=\"" + thisRow.link + getOptionalParams( thisRow ) + "\" class=\"browseItemNameContainer\">" + thisRow.name + "</a> " + fn_MenuStr( '', '', thisRow.dataId, '', imgSrc, LocString( 'Functions' ), '' ) + thisRow.modifiedImgs;
		}
		else
		{
			outputStr += "<SPAN class=\"browseItemNameContainer\">" + thisRow.name + "</SPAN>" + fn_MenuStr( '', '', thisRow.dataId, '', imgSrc, LocString( 'Functions' ), '' ) + thisRow.modifiedImgs;
		}
	...
	function makeSmallIconLink( thisRow )
	{
		...
		if ( thisRow.imgStatus.length !== 0)
		{
			...
			status = "<IMG SRC='" + thisRow.imgStatus + "' WIDTH='8' HEIGHT='8' BORDER='0' TITLE='" + statusName + "'>";
			
			if ( "" == thisRow.imgMapClassSmall )
			{
				outputStr += ahref;
				outputStr += "<IMG SRC='" + thisRow.imgStr + "' WIDTH='16' HEIGHT='16' BORDER='0'>";
				outputStr += status;				
			}
			else
			{
				...
				outputStr += "<DIV CLASS=\"" + thisRow.imgMapClassSmall + imgCursor + "\"></DIV>";				
			}
	...
	function class_buildTable()
	{
		...
		{
			killPopup();
			exceptionAlert( e, "Issue occured in classbrowse.js/buildTable.  An issue has occured in the creating the browse table." );
		}
	}
	...
	function class_updateViewType( newType )
	{
		var theDiv;
		var theTopRow;
		var	theBottomRow;
		...
		try
		{
			...
			{
				theDiv = document.getElementById('MultiOperationBar1_2');
				theTopRow = document.getElementById('MultiOperationBar1Row2');
				theBottomRow = document.getElementById("MultiOperationBar1RowBottom2");
				theDetailOperations = document.getElementById("MultiDetailOperations2");
				...
				theDiv=document.getElementById('MainOperationTable2');
				theDiv.style.display = 'block';
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/class_updateViewType.  An issue has occured in updating the browse view type." );
			changed = false;
		}
	...
	{ 
		...
		if ( c === 13 || c === 3 )
		{
			...
			if ( pickPageEdit !== null )
			{
				...
				if ( searchPage === undefined || searchPage == false )
				{
					class_getFolderContents( pageNum, String.formatLoc( "Retrieving page n", pageNum )  );
				}
				else
				{ 
	...
	function SortIndicatorGet( columnName )
	{
		...
		{
			if ( class_sortColumn === columnName )
			{
				gif = 'sort-up.gif';
				alt = LocString( "Ascending Sort" );
			}
			else if ( class_sortColumn === "-" + columnName )
			{
				gif = 'sort-down.gif';
	...
	function class_buildTableBigImage()
	{
		...
		var		cellCount = 0;
		var		theBigTbl = document.getElementById( "browseTableDefn2" );
		...
		try
		{
			...
			hasViewChanged = class_updateViewType( '2' );
			...
			setTimeout( class_showTablesNow, 1 );
		}
		catch(e)
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/buildTableBigImage.  An issue has occured in creating the large icon view." );
		}
	}
	...
	function class_buildTableListView()
	{
		var		cell;
		var		divElem2;
		var		divElem;
		...
		{
			// Set the viewType for the small icons view
			hasViewChanged = class_updateViewType( '3' );
			...
			totalHeight = Math.ceil( class_myRows.length / 2 );
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/buildTableListView.  An issue has occured in creating the small icon view." );
		}
	}

	// Build the Detail view.
	function class_buildTableDetail()
	{
		var		cell;
		var		divElem;
		var		hasViewChanged;
		...
		{
			// Set the viewType for the Detail view
			hasViewChanged = class_updateViewType( "1" );
			...
			for ( j = 0; j < class_columnDefinitionArray.length; j++ )
			{
				cellCount = class_columnDefinitionArray[j].AddCellHeader(row, class_myRows, cellCount);
			}

			divElem = document.createElement("div");
			divElem.id = 'itemView2';
			...
			for (i=0; i<numRows; i++)
			{
				...
				for ( j = 0; j < class_columnDefinitionArray.length; j++ )
				{
					cellCount = class_columnDefinitionArray[j].AddCell( row, thisRow, cellCount, i );
				}
				...
				if ( ( undefined !== thisRow.dcomment ) && ( 0 !== thisRow.dcomment.length ) )
				{
					...
					cell.colSpan = class_columnDefinitionArray.length - 1;
			...
			class_addPagingFooter( tbl, 6 );
			document.getElementById("browseCoreDiv2").appendChild(tbl);
			buildMiniTable( totalPages, currentPageNum );
			setTimeout( class_showTablesNow, 0 );			
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/buildTableDetail.  An issue has occured in creating the detail view." );
		}
	}
	...
	// DOM element.
	function class_drawBottomMultiButton()
	{
		var		buttonType;
		var		cell;
		var		innerHTMLStr;
		...
		try
				{
					row = document.getElementById("MultiOperationBar1RowBottom2");
					...
					if ( null === row )
					{
						...
						row = document.getElementById("BrowseCoreRow2");
						tbl = row.parentNode;
						...
						outerRow.id = "MultiOperationBar1RowBottom2";
						...
						innerHTMLStr = "<DIV ID=\"MultiDetailOperationsBottom2\" CLASS=\"MultiDetailOperations\">";
						fn_setDirectWrite( false );
		
						var lastMenuItem = 0;
						if ( 1 >= class_bcMultiButtonArray.length && null != class_bcMultiButtonArray[0].img1 )
						{
							for ( i = 0; i < class_bcMultiButtonArray.length; i++ )
							{
								if ((i + 1) == class_bcMultiButtonArray.length) {
									lastMenuItem = 1;
								}
								buttonType = class_bcMultiButtonArray[i];
						...
						var moreButton = document.getElementById("tMoreMenuPane2");
						...
						innerHTMLStr += "<DIV ID=\"tMoreMenuPaneBottom2\">" + replaceText + "</DIV></DIV>";
		...
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/drawBottomMultiButton.  An issue has occured in creating the bottom multi select operation buttons." );
		}
	}

	// Does a  sort of the current browse view by finding the BrowseViewColumn
	// object referred to by the global sortColumn and calling class_sortTableCol
	// on that object.
	function sortTable()
	{
		var		j;
	...
	// on the right object.
	function class_sortTableCol( headerClick, colIdentifier  )
	{		
		var i;
		...
		try
		{
			...
			if ( obj !== null )
			{
				return obj.class_sortTableCol( headerClick );
			}
		}
		catch(e)
		{
			exceptionAlert( e, "Issue occured in classbrowse.js/class_sortTableCol.  An issue has occured in trying to do a client-side sort. Sort Object - " + colIdentifier );
		}
	...
	function editRow(rowNo)
	{
		...
		else
		{
			...
			if ( true === class_myRows[rowNo].editable )
			{
				if ( isIE )
				{
				...
				if ( null !== theCell )
				{
					...
					var cellContent="<input type='text' id='editCell' class='valueEditableCell' value='" + class_myRows[rowNo].name + "'" + onKeyPressStr + " >";
	...
	function cellEditEnter( theForm, e )
	{
		...
		{
			// Up arrow
			if ( editingRow < class_myRows.length - 1 )
			{
				editRow( editingRow + 1 );
			}
			return false;
	...
	function class_folderContentsResponse( responseText )
	{
		class_DataStringToVariables( responseText );
	...
	function class_folderContentsSort( responseText )
	{
		class_DataStringToVariables( responseText );
	...
	function class_getFolderContents( pageNum, msg, responseMethod )
	{
		var url;
		...
		url = class_baseUrl + "?func=ll&objAction=page&objID=" + virtual_containerId + "&page=" + pageNum + "&sort=" + class_sortColumn + "&noGUI=true&classID="+ classId;
		...
		{
			OTsendHTTPGetAjaxRequest( url,  class_folderContentsResponse, msg );
		}
		else
		{
	...
	$(document).ready( function() {
	//	class_browseCoreInitialize();
	//	class_drawBottomMultiButton();
	//	class_buildTableBigImageResize();
	...
	function getFilterEditCellContent( updateContent )
	{
		...
		if ( filterValue !== newValue.value  )
		{
			...
			if ( updateContent )
			{
				...
				if( newValue.value == "" )
				{
					class_getFolderContents( 1, msg, class_folderContentsSort );
				}
				else
				{
					class_getFolderContents( 1, msg );
				}
			}
		}
	}
	...
	if (!class_bcMultiButtonArray)
	{
		var class_bcMultiButtonArray = [];
	}

	function class_multiButton( img1Str, img2Str, nameStr, displayNameStr, actionStr, typeStr )
	{
		this.img1 = img1Str;
		this.img2 = img2Str;
		this.name = nameStr;
	...
	function class_multiButton2( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, actionStr, typeStr )
	{
		this.imgMap = imgMap;
		this.xpos = xpos;
		this.ypos = ypos;
	...
	function class_multiButtonClient( img1Str, img2Str, nameStr, displayNameStr, jsFunc )
	{
		this.img1 = img1Str;
		this.img2 = img2Str;
		this.name = nameStr;
	...
	function class_multiButton2Client( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, jsFunc )
	{
		this.imgMap = imgMap;
		this.xpos = xpos;
		this.ypos = ypos;
	...
	function class_AddMultiButton( img1Str, img2Str, nameStr, displayNameStr, actionStr, typeStr )
	{
		class_bcMultiButtonArray[class_bcMultiButtonArray.length] = new  class_multiButton( img1Str, img2Str, nameStr, displayNameStr, actionStr, typeStr );
	}

	function class_AddMultiButton2( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, actionStr, typeStr )
	{
		class_bcMultiButtonArray[class_bcMultiButtonArray.length] = new  class_multiButton2( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, actionStr, typeStr );
	}
	
	function class_AddMultiButtonClient( img1Str, img2Str, nameStr, displayNameStr, jsFunc )
	{
		class_bcMultiButtonArray[class_bcMultiButtonArray.length] = new  class_multiButtonClient( img1Str, img2Str, nameStr, displayNameStr, jsFunc );
	}

	function class_AddMultiButton2Client( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, jsFunc )
	{
		class_bcMultiButtonArray[class_bcMultiButtonArray.length] = new  class_multiButton2Client( imgMap, xpos, ypos, xpos2, ypos2, nameStr, displayNameStr, jsFunc );
	}
	...
	function layoutToolbar() {
	  if ( currentViewType == '1' ) // Only support the detail view 
	  {
		...
		if (multiOperationBar1.length > 0) 
		{
			...
			var bottomToolbar = $("#MultiOperationBar1RowBottom2").find(".browseMultiSelectColor")[0];
	...
	{
		var		w = window.open( class_baseUrl + '?func=user.userdialog&userID=' + userID,'UserDialog','width=600,height=500,resizable=yes,scrollbars=yes,menubar=no');
	...
	$(document).ready(function(){
		...
		moreButtons.live("mouseover", function(){
...
var Sidebar = {
	initialize: function() {
		...
		this.outerContainer = $("#browseAndSidebarContainer2");
	...
	open: function(ignoreState) {
		...
		if( ( ignoreState == undefined ) || ( ignoreState == false ) ) {
			...
			$.ajax({
				type: "POST",
				cache: false,
				data: "func=ll.SaveSidebarState&state=1",
	...
	close: function(ignoreState) {
		...
		if( ( ignoreState == undefined ) || ( ignoreState == false ) ) {
			$.ajax({
				type: "POST",
				cache: false,
				data: "func=ll.SaveSidebarState&state=0",
				success: function(msg) {
					// For DEBUGGING response
				},
				error: function(XMLHttpRequest, textStatus, errorThrown) {
